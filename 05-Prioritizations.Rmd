...
Although we strongly recommend using [Gurobi](https://www.gurobi.com/) to solve problems (with [`add_gurobi_solver`](https://prioritizr.net/reference/add_gurobi_solver.html)), you can also use the [lpsymphony solver](https://prioritizr.net/reference/add_lsymphony_solver.html) in this workshop (it's much easier to install). The Gurobi solver is much faster than the lpsymphony solver ([see here for installation instructions](https://prioritizr.net/articles/gurobi_installation.html)).
...
```{r "budget-weights", out.width = "65%", warning = FALSE}
