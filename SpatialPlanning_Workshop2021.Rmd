...
Some of the new code in this workshop relies on _prioritizr R_ package v6.0 which is only available on Github at the moment. The CRAN version is currently 5.03. To install v6.0 you can run:
```{r, eval = FALSE}
...
to build a precise representation of the conservation planning problem required, and then solved to obtain a solutions.
...
# utm <- "+proj=utm +zone=55 +south +datum=WGS84 +units=m +no_defs +ellps=WGS84 +towgs84=0,0,0"

tas_pu <- readOGR("data/pu.shp")
tas_features <- stack("data/vegetation.tif")
proj4string(tas_pu) <- albers
proj4string(tas_features) <- albers

# tas_pu$locked_out[500:1130] <- FALSE
...
# tas_pu2 <- readOGR("data/pu.shp")
# tas_features2 <- stack("data/vegetation.tif")
# 
# proj4string(tas_pu2) <- utm
# proj4string(tas_features2) <- utm

# 
# tas_features3 <- projectRaster(tas_features2, crs = crs(utm))
# 
